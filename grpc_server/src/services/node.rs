...
impl NodeService for NodeServiceImpl {
    async fn status(
        ...
        let unchecked_count = self.node.unchecked.lock().unwrap().len() as u64;

        Ok(Response::new(StatusResponse {
            version: "1".to_string(),
            synced: true,
            bootstrap_storage: false,
            block_count: self.node.ledger.block_count(),
            cemented_count: self.node.ledger.confirmed_count().to_string(),
            unchecked_count,
            account_count: self.node.ledger.account_count(),
            bandwidth_cap: 0,
            peer_count,
    ...
    async fn version(
        ...
        Ok(Response::new(VersionResponse {
            ...
            store_version: self.node.ledger.version().to_string(),
            protocol_version: self
                .node
                .network_params
                .network
